...
func (r *MongoDBClusterReconciler) reconcileConfigServer(ctx context.Context, log logr.Logger, mgoCluster *mongodbv1.MongoDBCluster) (ctrlRet ctrl.Result, retErr error) {
	...
	if ctrlRet, retErr = r.reconcileConfigServerHeadlessService(ctx, log, mgoCluster); retErr != nil {
		return
	}
...
func (r *MongoDBClusterReconciler) reconcileConfigServerHeadlessService(ctx context.Context, log logr.Logger,
	mgoCluster *mongodbv1.MongoDBCluster) (ctrl.Result, error) {

	confSrvSpec := mgoCluster.Spec.ConfigServer

	var svc corev1.Service
	name := fmtComponentTypeObjectName(mgoCluster.GetName(), mongodbv1.ComponentTypeConfigServer, confSrvSpec.ReplicaSetId)
	...
	if err := r.Get(ctx, key, &svc); err != nil {
		...
		log.Info("Successfully created a headless service for %v %v", mongodbv1.ComponentTypeConfigServer, confSrvSpec.ReplicaSetId)
...
func (r *MongoDBClusterReconciler) reconcileConfigServerStatefulSet(ctx context.Context, log logr.Logger,
	...
	if foundStatefulSet.Spec.Replicas == nil || (*foundStatefulSet.Spec.Replicas) != confSrvSpec.NumReplicaSetNodes {
		foundStatefulSet.Spec.Replicas = ptr.To(confSrvSpec.NumReplicaSetNodes)
...
func (r *MongoDBClusterReconciler) newConfigServerStatefulSet(mgoCluster *mongodbv1.MongoDBCluster) (*appsv1.StatefulSet, error) {
	...
	objectName := fmtComponentTypeObjectName(mgoCluster.GetName(), mongodbv1.ComponentTypeConfigServer, confSrvSpec.ReplicaSetId)
	statefulSet := &appsv1.StatefulSet{
		...
		Spec: appsv1.StatefulSetSpec{
			Replicas: ptr.To(confSrvSpec.NumReplicaSetNodes),
			Selector: &metav1.LabelSelector{
				MatchLabels: map[string]string{"component-type": string(mongodbv1.ComponentTypeConfigServer)},
			},
			ServiceName: fmtComponentTypeObjectName(mgoCluster.GetName(), mongodbv1.ComponentTypeConfigServer, confSrvSpec.ReplicaSetId),
